...
function CrimeGenreList() {
  ...
  const getAnimeList = () => {
    GlobalApi.getMovieByGenreId(10752) // Assuming 16 is the genre ID for Animation
      .then((resp) => {
      ...
      >
        {animeList.map((anime, index) => (
          <React.Fragment key={index}>
            ...
            >
              <SeriesCard movie={anime} imageWidth={400} imageHeight={600} />{" "}
              <br />
            </div>
          </React.Fragment>
        ))}
